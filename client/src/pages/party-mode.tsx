...
import {
  ...
  Sparkles, Music, Zap, ChevronRight, ChevronLeft,
} from "lucide-react";
...
function generatePartySound(ctx: AudioContext, type: string, freq: number) {
  ...
  switch (type) {
    ...
    case "crowd": {
      ...
      src.connect(masterGain); src.start(now); break;
    }
  }
}

type PartySection = "mix" | "fx" | "mic";
...
export default function PartyMode() {
  ...
  const handleFileA = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) { engine.loadFile(file, "A"); setShowFirstTimeTip(false); }
  }, [engine]);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col max-w-lg mx-auto w-full px-3 pt-2 pb-4 gap-3 overflow-auto">
        {showFirstTimeTip && (
          <TipBubble text="Welcome! Tap 'Load' on a deck to add a song, then hit Play. It's that easy! 🎉" />
        )}

        <div className="flex gap-3 justify-center">
          {[
            { id: "A" as const, color: "#ff2d78", fileRef: fileInputARef, onFile: handleFileA, deck: deckA },
            { id: "B" as const, color: "#0af", fileRef: fileInputBRef, onFile: handleFileB, deck: deckB },
          ].map(({ id, color, fileRef, onFile, deck }) => (
        ...
        <div className="glass-panel rounded-2xl p-4" style={{ borderColor: "rgba(191,90,242,0.15)" }}>
          ...
          <div className="relative">
            <input
              ...
              style={{
                background: `linear-gradient(to right, #ff2d78 ${crossfade * 100}%, #0af ${crossfade * 100}%)`,
              }}
        ...
        {activeSection === "mix" && (
          <div className="space-y-3 animate-slide-in-up">
            ...
            <TipBubble text="Tip: Slide the Blend bar to mix between songs! Center = both play equally, far left/right = only one song plays." />
          </div>
        )}
        ...
        {activeSection === "mic" && (
          <div className="animate-slide-in-up space-y-3">
            ...
            <TipBubble text="Turn on your mic to speak over the music! Great for hyping up the crowd, making announcements, or just having fun 🎙️" />
          </div>
        )}
      </div>
